...
export class UILayer {
  ...
  updateConfidence(score) {
    if (this.$confValue) {
      this.$confValue.textContent = score !== null ? `${(score * 100).toFixed(0)}%` : '--';
    }
  }
  ...
  updateMetrics(metrics) {
    ...
    if (this.$mConfidence) this.$mConfidence.textContent = metrics.confidence   ?? '--';
  ...
  setUndoEnabled(v) { this.$undoBtn?.toggleAttribute('disabled', !v); }
  setRedoEnabled(v) { this.$redoBtn?.toggleAttribute('disabled', !v); }
  ...
  _bindKeyboard() {
    document.addEventListener('keydown', (e) => {
      if (e.ctrlKey || e.metaKey) {
        if (e.key === 'z') { e.preventDefault(); this.onUndo?.(); }
        if (e.key === 'y') { e.preventDefault(); this.onRedo?.(); }
        if (e.key === 's') { e.preventDefault(); this.onSave?.(); }
      }
